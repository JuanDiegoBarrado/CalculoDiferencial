...
\begin{prop}
En $(\mathbb{R}^n, || \ ||_2)$ espacio euclídeo usual, la convergencia de sucesiones se puede caracterizar como:
$$\{x_m\}_{m \in  \mathbb{N}}: x_m = (x_m^1, \cdots, x_m^n) \stackrel{n\rightarrow\infty}{\longrightarrow} x = (x^1, \ldots, x^n) \Leftrightarrow \forall j = 1, \cdots, n: x_m^j \xrightarrow{m\rightarrow\infty} x^j$$
Es decir, se reduce a la convergencia de sucesiones en $\mathbb{R}$ sobre cada coordenada.
\end{prop}
...
\begin{enumerate}
\item Un subconjunto $A \subset E$ es cerrado si y sólo si $\forall \{x_n\}_{n\in \mathbb{N}} \subset A : \left(\exists \lim_{n \rightarrow \infty} x_n = x \Rightarrow x \in A\right)$
...
\begin{prop}
Sea $(E,d)$ un espacio métrico, podemos caracterizar de nuevo la convergencia. Sea $\{x_n\}_{n\in \mathbb{N}} \subset E$
\begin{enumerate}
...
$$\forall \varepsilon > 0, \exists n_0 \in \mathbb{N} : \forall m,n : m > n \geq n_0 \Rightarrow d(x_m, x_n) < \varepsilon$$
...
En $(E,d)$, un conjunto $A \subset E$ es \textbf{acotado} si
